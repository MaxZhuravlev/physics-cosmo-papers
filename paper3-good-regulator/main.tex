...
Can all of physics emerge from a single axiom? This foundational question drives the cosmological unification program, which seeks to demonstrate that causal invariance---the substrate-independent consistency of causal structure---constrains physical law through established uniqueness theorems.
...
A companion paper~\cite{companion_lovelock} establishes the \emph{Lovelock bridge}: if the continuum limit holds, causal invariance constrains Vanchurin's Onsager tensor symmetries to produce Einstein's field equations. This synthesis demonstrates that \emph{geometry forces gravity} through uniqueness.
...
\begin{enumerate}[noitemsep]
\item \textbf{Continuum Limit:} Like Paper \#1 (Lovelock Bridge), we assume the continuum limit of hypergraph evolution exists and is well-behaved. This is unproven and may not hold rigorously.
...
\begin{itemize}[noitemsep]
\item \textbf{Paper \#1 (Lovelock Bridge):} Causal invariance $\to$ Einstein equations (via Lovelock uniqueness)
