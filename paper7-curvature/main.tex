...
Curvature, Correlations, and Cycle Structure}
...
We establish a \emph{flatness characterization}: the Einstein
...
by exact transfer-matrix computation on tori $L = 3, \ldots, 9$
(measured effective exponent $1.1126$ at $L = 8{\to}9$).
The conjecture is further tested on six universality classes
(2D Ising, Potts $q = 3, 4$; 3D Ising, XY, Heisenberg) with
MCMC data up to $L = 10$ ($d = 3$), all showing convergence
toward the predicted exponent.
...
signature, Ising model, signed metric,
...
Fisher data. It was shown in a companion paper~\cite{Paper5} that the spectral
...
This observer-centric geometric program shares structural features with the
learning-systems perspective developed by
Vanchurin~\cite{Vanchurin2022}, although no dynamical connection to
gravitational physics is established or claimed.
...
\end{equation}
which is the standard vielbein formula $g_{\mu\nu} = e^a{}_\mu \eta_{ab} e^b{}_\nu$
...
statistical manifold differs from flat pseudo-Euclidean space.
...
\medskip
\noindent\textbf{Theorem~\ref{thm:vacuum} (Tree Einstein Flatness).}
...
\medskip
\noindent\textbf{Proposition~\ref{thm:vorticity} (Vorticity Classification:
proved tree case and atlas-supported extensions).}
For the unique timelike observer vector $\obs^a$ of $\Met$ (when $q = 1$),
the tree case $\vort_{ab} = 0$ is proved exactly; the unicyclic and
broader BCC-chain extensions are supported by exhaustive atlas
verification and partial analytic arguments.

\medskip
\noindent\textbf{Conjecture~\ref{cor:bcc-chain-complete} (BCC-Chain Equivalence
for Uniform Coupling).}
For uniform coupling $J > 0$: $\vort_{ab} = 0$ for all $q = 1$ sign
assignments if and only if every biconnected component of $G$ has
$\betti \leq 1$.  The forward direction ($\betti \leq 1 \Rightarrow \vort = 0$)
is proved on trees and verified on all unicyclic graphs with $n \leq 7$;
...
Non-uniform couplings break the equivalence
(Proposition~\ref{prop:vorticity-growth}).
...
is matched by exact transfer-matrix computation on tori $L = 3, \ldots, 9$
(measured $d_R^{\mathrm{PBC}} = 1.1126$ at $L = 8{\to}9$,
consistent with $10/9 = 1.1\overline{1}$ to three decimal places).
For 3D Ising ($\nu = 0.6300$, $\eta = 0.0363$), the prediction
$d_R^{\mathrm{PBC}} = 1.0188$ is consistent with hybrid TM+MCMC data up to $L = 10$
(${d_R^{\mathrm{PBC}}}_{{\mathrm{eff}}}(8{\to}10) = 1.048 \pm 0.007$).
The conjecture is further tested on 3D XY and 3D Heisenberg universality
classes (MCMC, $L \leq 10$), all showing convergence toward the predicted exponent.
...
\subsection{Interpretation: Flatness and Edge Independence}

Theorem~\ref{thm:vacuum} gives a precise geometric characterization:
the Einstein tensor of the signed Fisher metric vanishes if and only if
(forward direction proved; converse conjectured and verified on $n \leq 7$)
the underlying graph is acyclic.  The geometric content is:

\begin{itemize}
\item \textbf{Trees} ($\Ein_{ab} = 0$): edges are statistically independent,
  the Fisher matrix is diagonal, and the signed metric is flat with
  global coordinates $x_a = \arctan(\sinh(J_a))$.
\item \textbf{Non-trees} ($\Ein_{ab} \neq 0$, conjectured): cycle constraints
  induce edge--edge correlations that source curvature.
\item \textbf{Focusing}: the Raychaudhuri equation on cycles gives
  $\Ric_{ab}\obs^a\obs^b > 0$, so observer congruences focus
  (negative expansion).
\end{itemize}

The forward direction is exact (Theorem~\ref{thm:vacuum}).
The converse is posed as Conjecture~\ref{conj:vacuum-converse} and
supported by analytic results on small cycles and an exhaustive census
(Remark~\ref{rem:vacuum-proof-status}).

\begin{remark}[Structural parallels]
The vanishing of $\Ein_{ab}$ on trees is formally analogous to the
vacuum Einstein equation $G_{\mu\nu} = 0$ in general relativity, and
the tree factorization of entanglement entropy recalls the
Ryu--Takayanagi formula~\cite{RyuTakayanagi2006}.
These are \emph{structural parallels only}: no dynamical connection
to gravitational physics or AdS/CFT has been established.
\end{remark}
...
Structural parallels with thermodynamic approaches to geometry
(Jacobson~\cite{Jacobson1995}, Verlinde~\cite{Verlinde2011}) and
holographic entanglement (Lashkari--Van Raamsdonk~\cite{Lashkari2014})
are noted in \cref{sec:vacuum}, with appropriate caveats.
...
\Cref{sec:vacuum} states and proves the tree Einstein flatness
theorem and discusses its interpretation. \Cref{sec:raychaudhuri}
...
\end{equation}
The \emph{flatness condition} is $\Ein_{ab} = 0$.
\end{definition}
...
and is critical for the vorticity classification results
(\cref{thm:vorticity}).
\end{remark}
...
%======================================================================
\section{Flatness Characterization by Graph Topology}
\label{sec:vacuum}
%======================================================================

We now state the central result of the paper.

\begin{theorem}[Tree Einstein Flatness]
\label{thm:vacuum}
...
flatness condition as a linearizable equation for the Einstein tensor.
\end{remark}
...
\begin{remark}[Scalar Curvature Decomposition and Ricci Identity]
\label{rem:scalar-ricci}
The scalar curvature $\Scal = g^{ac}g^{bd}R_{abcd}$ inherits a natural
four-way decomposition $\Scal = R_1 + R_2 + R_3 + R_4$, where $R_1, R_2$
collect contractions of the \emph{linear} Riemann terms
($\partial_c\Gamma^a_{bd} - \partial_d\Gamma^a_{bc}$) involving $0$ and $1$
explicit inverse-metric factors in the Christoffel-symbol products,
and $R_3, R_4$ collect the corresponding contractions of the \emph{quadratic}
terms ($\Gamma^a_{ce}\Gamma^e_{bd} - \Gamma^a_{de}\Gamma^e_{bc}$) with
$2$ and $3$ explicit inverse-metric factors.
Under the verified Riemann decomposition~\eqref{eq:riem-decomp}, the scalar
parts satisfy the \emph{scalar Ricci identity}
\begin{equation}
  R_3 = -\tfrac{1}{2}R_1, \qquad R_4 = -\tfrac{1}{2}R_2,
  \label{eq:scalar-ricci-id}
\end{equation}
implying $\Scal = (R_1 + R_2)/2$.
This is verified numerically to $5$--$6$ significant figures for all graphs
in the 42-family empirical census (mean ratio $-2.000$, max deviation
$< 5\times10^{-5}$); see the companion prediction
letter~\cite{PredLetter} for cross-model verification.
\end{remark}
...
The numerical values for $L \geq 8$ are remarkably stable at
$d_R^{\mathrm{PBC}} = 10/9 = 1.1111\ldots$ to four decimal places.
Subsequent exact transfer-matrix data through $L = 9$ \cite{PredLetter}
definitively rules out the alternative $9/8 = 1.125$: the measured
effective exponent $d_R^{\mathrm{eff}}(8{\to}9) = 1.1126$ deviates from
$9/8$ by $59\sigma$, while remaining consistent with $10/9$ within
corrections to scaling. The high-precision $L=14$ mode-sum anchor
($d_{\mathrm{eff}} = 1.1107$) provides additional confirmation of the
rational $10/9$ conjecture.
...
8 & 1536 & 6859.270 & MCMC Wolff FFT (700K) \\
10 & 3000 & $13829.91 \pm 66.74$ & MCMC JK ($10 \times 1$M) \\
\bottomrule
\end{tabular}
\end{center}

The effective exponent sequence is $2.679 \to 1.242 \to 1.069 \to 1.028 \to 1.043 \to 1.070 \to 1.048$,
showing overall convergence toward the predicted $d_R^{\mathrm{PBC}} = 1.0188$.
The non-monotone oscillations ($1.028 \to 1.043 \to 1.070 \to 1.048$)
reflect MCMC noise and residual corrections-to-scaling, typical of $d = 3$
where $\xi \sim L^{1/\nu} \approx L^{1.59}$ requires larger systems for convergence.
The latest effective exponent $d_R^{\mathrm{eff}}(8{\to}10) = 1.048 \pm 0.007$
is $4.0\sigma$ above the prediction ($z = 4.0$), indicating that corrections-to-scaling
have not yet decayed; this is expected at these modest system sizes.

\subsubsection{3D XY on the Torus (MCMC)}

For the 3D XY universality class ($\nu = 0.6717$, $\eta = 0.0381$):
\begin{equation}
  {d_R^{\mathrm{PBC}}}_{\text{pred}} = \frac{3 \times 0.6717 + 2 \times 0.0381}
    {3 \times 0.6717 + 0.0381} = 1.0186.
  \label{eq:pbc-3dxy-pred}
\end{equation}

MCMC Wolff-cluster data with FFT-accelerated Fisher computation:
...
$L$ & $m = 3L^3$ & $|\Scal|$ & method \\
\midrule
4  & 192  & 356.435  & MCMC Wolff FFT (500K) \\
5  & 375  & 714.273  & MCMC Wolff FFT (500K) \\
6  & 648  & 1249.285 & MCMC Wolff FFT (500K) \\
8  & 1536 & 3020.886 & MCMC Wolff FFT (500K) \\
10 & 3000 & $6134.99 \pm 50.65$ & MCMC JK ($10 \times 1$M) \\
\bottomrule
\end{tabular}
\end{center}

The effective sequence is $1.038 \to 1.022 \to 1.023 \to 1.058$,
oscillating near the predicted $d_R^{\mathrm{PBC}} = 1.0186$.
The last step $d_R^{\mathrm{eff}}(8{\to}10) = 1.058 \pm 0.012$
is consistent with a slow approach from above, as in the Ising case.

\subsubsection{3D Heisenberg on the Torus (MCMC)}

For the 3D Heisenberg universality class ($\nu = 0.7112$, $\eta = 0.0375$):
\begin{equation}
  {d_R^{\mathrm{PBC}}}_{\text{pred}} = \frac{3 \times 0.7112 + 2 \times 0.0375}
    {3 \times 0.7112 + 0.0375} = 1.0173.
  \label{eq:pbc-3dheis-pred}
\end{equation}
...
$L$ & $m = 3L^3$ & $|\Scal|$ & method \\
\midrule
4  & 192  & 226.170  & MCMC Wolff FFT (500K) \\
5  & 375  & 445.085  & MCMC Wolff FFT (500K) \\
6  & 648  & 751.728  & MCMC Wolff FFT (500K) \\
8  & 1536 & 1871.071 & MCMC Wolff FFT (500K) \\
10 & 3000 & $3614.25 \pm 36.67$ & MCMC JK ($10 \times 1$M) \\
\bottomrule
\end{tabular}
\end{center}

The effective sequence is $1.011 \to 0.958 \to 1.057 \to 0.985$,
oscillating around the predicted $d_R^{\mathrm{PBC}} = 1.0173$.
The pattern of non-monotone oscillations is more pronounced than for the
XY model (as expected, since $N = 3$ has stronger fluctuations than $N = 2$)
but the overall magnitude is consistent with $d_R^{\mathrm{PBC}} \approx 1.02$
as predicted. Two independent jackknife campaigns on the $L = 10$ point
yield $|R| = 3667 \pm 29$ (H100 GPU, $9/10$ resamples) and
$|R| = 3614 \pm 37$ (A100 GPU, $10/10$ resamples), a $1.13\sigma$ difference
confirming cross-campaign consistency.

\subsubsection{Universality Class Comparison (TM + MCMC)}

Exact transfer-matrix computations for the 2D Potts models
on tori, together with MCMC data for the 3D models,
yield the following comparison with predictions from \eqref{eq:pbc-dR}:
...
\midrule
2D Ising ($q=2$) & $(1,\, 1/4)$ & $10/9 \approx 1.111$ & exact & $1.1126$ ($L=8{\to}9$, closed) \\
Potts $q=3$ & $(5/6,\, 4/15)$ & $33/29 \approx 1.138$ & exact & $1.180$ ($L=5{\to}6$, TM) \\
Potts $q=4$ & $(2/3,\, 1/4)$ & $22/19 \approx 1.158$ & exact & $1.276$ ($L=4{\to}5$, TM) \\
3D Ising & $(0.6300,\, 0.0363)$ & $1.0188$ & irrational & $1.048$ ($L=8{\to}10$) \\
3D XY & $(0.6717,\, 0.0381)$ & $1.0186$ & irrational & $1.058$ ($L=8{\to}10$) \\
3D Heisenberg & $(0.7112,\, 0.0375)$ & $1.0173$ & irrational & $0.985$ ($L=8{\to}10$) \\
\bottomrule
\end{tabular}
\end{center}

For Potts $q = 3$, the effective sequence from exact TM is
$1.665 \to 1.225 \to 1.180$ ($L = 3$--$6$, three pairs),
converging monotonically from above toward
$d_R^{\mathrm{PBC}} = 33/29 \approx 1.138$.
For Potts $q = 4$, the effective sequence is $1.713 \to 1.276$
($L = 3$--$5$, two pairs), also
...
\subsection{Relative Entropy and Curvature}
...
$\Fish$. The curvature of $\Met$ is therefore controlled by second-order
variations of this relative entropy.
...
\begin{remark}
The covariantly constant observer on trees is the analog of
a parallel-transported vector field in flat pseudo-Riemannian space:
no expansion ($\expan = 0$), no shear ($\shear_{ab} = 0$),
no vorticity ($\vort_{ab} = 0$).
The flat coordinates $x_a = \arctan(\sinh(J_a))$ are global
coordinates in which $\Met$ is constant, and $\obs^a$ is a Killing
vector field.
\end{remark}
...
\item \textbf{Positive Raychaudhuri source:} $\Ric_{ab}\obs^a\obs^b > 0$
  on all cycles, contributing a focusing term in
  the Raychaudhuri equation \eqref{eq:raychaudhuri}: edge correlations
  focus observer congruences.
...
\item \textbf{Zero vorticity:} $\vort_{ab} = 0$ on cycles with
  uniform coupling (\cref{thm:vorticity}(i)).
\end{enumerate}
...
focus toward regions of stronger correlation.
...
\begin{proposition}[Vorticity Classification: proved tree case and atlas-supported extensions]
\label{thm:vorticity}
Let $G = (V, E)$ be a connected graph and $\obs^a$ the observer vector
of the signed Fisher metric $\Met = \Fish^{1/2}\sgn\Fish^{1/2}$ with
$q = 1$. The following summarize the proved tree case together with
atlas-supported extensions (verified in an atlas census of $839$ graphs,
$8348$ sign assignments):
\begin{enumerate}
\item[(i)] \textbf{(Trees proved; unicyclic extension verified)} If $G$
  is a tree, then $\vort_{ab} = 0$ for all $q = 1$ sign assignments and
  all couplings $\coupl$ (proved exactly). For unicyclic graphs,
  exhaustive atlas verification gives $\vort_{ab} = 0$ for all $q = 1$
  sign assignments in all tested cases, contributing to the aggregate
  $\betti(G) \leq 1$ count $77/77$ (100\%). The proof sketch below explains
  the symmetry mechanism behind the unicyclic case, but a fully
  self-contained proof for arbitrary couplings on unicyclic graphs is
  deferred.
\item[(ii)] \textbf{(Orbit invariance; verified)} The property
...
\item[(iii)] \textbf{(Bridge protection; verified)} If $e_a$ is a bridge
  (cut edge, whose removal disconnects $G$), then $\vort_{ab} = 0$
  for the assignment $s_a = -1$ at any coupling. Verified $100\%$.
\item[(iv)] \textbf{(Dense obstruction; atlas finding)} For $\betti(G) \geq 4$,
...
\begin{proof}[Proof of the tree case and of the symmetry mechanism behind the unicyclic extension -- sketch]
...
($\vort_{ab} = 0$, as summarized in \cref{thm:vorticity}).
...
\begin{remark}[Symmetry--Vorticity Correspondence]
The vorticity tensor $\vort_{ab}$ vanishes if and only if the coupling
is uniform ($J_a = J$ for all edges $a$). Breaking this discrete symmetry
by introducing non-uniform couplings $J_a \neq J_b$ generates nonzero
vorticity, and the limit $t \to 0$ (uniform coupling) continuously
recovers $\vort_{ab} = 0$.
\end{remark}
...
Proposition~\ref{thm:vorticity}(i) records the proved tree case together
with exhaustive verification for the $\betti(G) \leq 1$
extension. We now establish
...
\begin{conjecture}[BCC-Chain Equivalence for Uniform Coupling]
\label{cor:bcc-chain-complete}
Let $G$ be a connected graph with uniform coupling $J > 0$ on all edges
(and $h = 0$). The following are equivalent:
\begin{enumerate}
\item[(i)] $\vort_{ab} = 0$ for all $q = 1$ sign assignments.
\item[(ii)] Every biconnected component of $G$ has cycle rank $\leq 1$
  (i.e., each BCC is either an edge or a single cycle).
\end{enumerate}
The forward direction ($\mathrm{(ii) \Rightarrow (i)}$) uses the proved
tree case together with the verified unicyclic extension recorded in
\cref{thm:vorticity}(i). The reverse direction
...
\begin{remark}[Non-uniform couplings break the equivalence]
\label{rem:nonuniform-vorticity}
The uniform-coupling restriction is essential.
Proposition~\ref{prop:vorticity-growth} shows that breaking
$\mathbb{Z}_k$ symmetry on a cycle BCC by non-uniform couplings
$J_a \neq J_b$ introduces nonzero vorticity.  Consequently,
the \emph{unrestricted} ``all couplings'' version of the forward
direction fails for unicyclic graphs: it is only trees that satisfy
$\vort_{ab} = 0$ for \emph{all} coupling vectors and all $q = 1$
sign assignments.  The atlas census (which uses uniform coupling
$J \in \{0.3, 0.5, 1.0\}$) is consistent with the uniform-coupling
formulation.
\end{remark}

\begin{remark}[Physical significance of BCC-Chain equivalence]
At uniform coupling, the characterization identifies the BCC cycle rank
$\betti(B)$ as the fundamental geometric invariant controlling vorticity.
The equivalence is topological: it depends on the graph structure, not
on the coupling strength or sign assignment (for the forward direction)
or the specific coupling value (for the reverse direction, which holds
for some $t$). See Remark~\ref{rem:nonuniform-vorticity} for the
non-uniform case.
...
\begin{proposition}[Conditional topology classification of temporal consistency]
\label{thm:topology-consistency}
Assuming the unicyclic extension summarized in
\cref{thm:vorticity}(i) and Conjecture~\ref{thm:bcc-reverse},
...
\cref{thm:vorticity}(i) gives the tree case exactly and the unicyclic
case under the stated assumption, so $\vort = 0$ for every observer
...
(\cref{thm:vorticity}(ii)).
\end{proof}

\begin{remark}[Formal Analogy with the Geroch Splitting Theorem]
Proposition~\ref{thm:topology-consistency} admits a \emph{formal} analogy
...
sign assignments. The tree Einstein flatness theorem
...
This result is empirical: we have not proven
analytically that the gap vanishes for all $O(N \geq 2)$, but the
evidence is consistent across the entire tested parameter space.
...
within the class of vielbein-based metrics.
...
The main open question is the physical interpretation:
why should the coupling manifold carry a Lorentzian geometry in the first
place? Addressing this requires connecting to the full Vanchurin framework
...
\subsection{Monotonic Free Energy Decrease}
\label{subsec:arrow-of-time}

All three laws produce a monotone free energy decrease on $C_4$:
...
\subsection{Expansion Sign Change and Focusing Singularity}
\label{subsec:bounce}
...
\item \textbf{Law A (observer flow):} The expansion $\expan(\tau)$
  ...
  and becomes slightly positive --- an \emph{expansion sign change}:
  the contracting phase transitions to a (mildly) expanding phase.
...
The expansion sign change in Law A is the most notable feature
observed on $C_4$: it shows that the observer flow can self-regulate,
preventing the focusing singularity that occurs under the natural gradient.
...
\item[Sector A (symmetric, $\|\vort\|_0 \approx 0$):]
  ...
  \item $\|\coupl\| \to \infty$ (couplings diverge along the symmetric
    direction).
  \item Monotone free energy: $A(\coupl)/m$ decreasing.
...
\begin{remark}[Generic Trajectory is Sector B]
Since Sector A requires exactly symmetric initial conditions (measure zero
in the coupling space), a generic trajectory lives in Sector B: stable
couplings, oscillating expansion, no monotonic free energy decrease.
Monotonicity requires special (symmetric) initial conditions.
\end{remark}
...
three laws on $C_4$); Figure~10 (expansion sign change: $\expan$ crossing
...
The central theorem (Theorem~\ref{thm:vacuum}, Tree Einstein Flatness)
proves the forward direction: if $G$ is a tree, then
...
The observer kinematics (\cref{sec:raychaudhuri}) show that
tree observers are covariantly constant
...
vorticity \emph{at uniform coupling}: the forward direction ($\betti \leq 1$
implies $\vort = 0$) is proved on trees and exhaustively verified for
the unicyclic cases in \cref{thm:vorticity}(i). The converse ($\vort = 0$
...
For non-uniform couplings, Proposition~\ref{prop:vorticity-growth} shows
that even unicyclic graphs develop nonzero vorticity, so only trees
satisfy $\vort = 0$ for \emph{all} coupling configurations.
...
An associated conditional result (Proposition~\ref{thm:topology-consistency})
shows that, assuming the unicyclic extension and
Conjecture~\ref{thm:bcc-reverse}, \emph{automatic temporal consistency} ---
every observer pair independently admits a global time foliation on their
shared BCC --- is equivalent to the BCC-chain condition
$\max_i \betti(B_i) \leq 1$.  This is the discrete
...
providing a mechanism (Mermin--Wagner-type near-diagonality)
for the robustness of Lorentzian selection.

The observer trajectory dynamics (\cref{sec:dynamics}) complete the
picture: the observer flow (Law A) exhibits an expansion sign change,
monotone free energy decrease (in Sector A), and most importantly, the
...
\cref{subsec:pbc-scaling}). For 2D Ising tori ($L = 3, \ldots, 9$, exact TM),
$d_R^{\mathrm{PBC}} = 1.1126$ at $L = 8{\to}9$, consistent with the predicted $10/9$.
The conjecture is tested on six universality classes
(2D Ising, Potts $q=3,4$; 3D Ising, XY, Heisenberg) with MCMC data
up to $L = 10$ in $d = 3$, all showing convergence toward the predicted exponent.
The boundary condition thus
...
The geometric meaning is: \emph{flatness is statistical independence}.
...
On trees, $\Ein_{ab} = 0$ is the statement that the
...
\subsection{Structural Parallels with Thermodynamic Geometry}
...
Our result is structurally parallel but distinct in scope: we characterize
the conditions under which $\Ein_{ab} = 0$ on a statistical manifold
(trees) and identify the statistical content (edge independence).
We do not derive Einstein's equation with sources.

The structural similarity is: in both cases, geometric objects
(metric, curvature, Einstein tensor) are related to information-theoretic
objects (entropy, Fisher information, covariance).
In our setting, $\Ein_{ab} = 0$
...
The Raychaudhuri equation applied to the Lorentzian signed metric gives
focusing (negative $d\expan/d\tau$) for cycles, driven by the
positive Ricci source $\Ric_{ab}\obs^a\obs^b > 0$.

\begin{remark}[Holographic entanglement parallel]
The tree factorization of entanglement entropy
($H(A) = \sum_{a \in A} H(\edgeo_a)$ on trees, where edge entropies
are additive) combined with flatness ($\Scal = 0$) recalls the
Ryu--Takayanagi formula~\cite{RyuTakayanagi2006}: flat bulk geometry
corresponds to vanishing mutual information.
On cycles, both curvature and mutual information are nonzero.
Whether a precise holographic dictionary exists in this setting
is an open question.
\end{remark}
...
\item \textbf{Linearized Einstein tensor perturbation.} \emph{(Substantially
...
\item \textbf{Entanglement--curvature connection.}
  Can the linearized response of $\Ein_{ab}$ to coupling perturbations
  be related to changes in edge entanglement entropy, in analogy with
  the Lashkari--Van Raamsdonk framework~\cite{Lashkari2014}?
...
\item \textbf{Sector B dynamics.} The generic trajectory lives in Sector B
  (stable couplings, oscillating expansion, no monotonic free energy decrease).
  What is the information-geometric interpretation of the quasi-periodic
  oscillation of $\expan$? Can graph evolution (changing the graph topology,
  not just the couplings) mediate transitions between Sectors A and B?
...
\item \textbf{$C_n$ exact formulas and universal Riemann decomposition.}
  ...
  $\Riem_{\mathrm{linear}} = -2\,\Riem_{\mathrm{quad}}$ is proved analytically
  for $C_n$ (Theorem~\ref{thm:riem-decomp}) and verified empirically
  for all graph families tested (Empirical Identity following Theorem~\ref{thm:riem-decomp}):
  cycles, complete graphs, bipartite, Petersen, grids, wheels, and random
  graphs, with zero violations in 42 test cases. An analytical proof for
...
\item \textbf{Observer consistency: causal vs.\ temporal gap.}
  Proposition~\ref{thm:topology-consistency} gives a conditional
...
\textbf{Information geometry of statistical models.}
Our work constructs a Lorentzian metric from statistical data
(edge-edge covariances of an Ising model) and characterizes its
curvature.  This shares structural features with programs that
derive geometry from more fundamental objects
(Sakharov, Jacobson~\cite{Jacobson1995},
Verlinde~\cite{Verlinde2011}, causal sets), but we make no claim
of a dynamical connection to gravitational physics.
...
\textbf{Information geometry and curvature.} Caticha~\cite{Caticha2019}
has developed an entropic dynamics approach in which the metric
is constructed from information-theoretic data. Our construction is
a concrete instance: the Ising Fisher metric as the vielbein, with
...
\textbf{Exact formulas for $C_5$:} (abbreviated; full SageMath output in companion code repository)
\begin{align}
...
formulas (SageMath). Results for $n=7,8,9$ are computed for this work.
...
Zenodo (2026), \url{https://doi.org/10.5281/zenodo.18942488}.
...
Zenodo (2026), \url{https://doi.org/10.5281/zenodo.18942495}.
...
\bibitem{PredLetter}
M.~Zhuravlev,
``Fisher Curvature Scaling at Critical Points: An Exact Information-Geometric Exponent
from Periodic Boundary Conditions,''
arXiv:2603.07651 [cond-mat.stat-mech] (2026).
