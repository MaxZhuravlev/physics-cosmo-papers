...
\title{Fisher Metric Asymptotics for the 2D Ising Model:\\
A Physics Derivation of the $d_{\mathcal{R}}$ Exponent}
...
\begin{abstract}
We derive the large-$L$ asymptotics of the dominant eigenvalue of the
Fisher information metric on the microscopic coupling space of
...
uniform remainder estimates, we derive (conditional on a standard
finite-size RG assumption for the $\sigma$-sector vertex function;
see Remark~\ref{rem:proof-status}) that the dominant eigenvalue
...
$\alpha_f = 2/9$ and provides the dominant eigenvalue
scaling that enters the curvature scaling law
$|\Scal(\Jc)| \sim n^{\dR}$ with $\dR = 10/9$.

Numerical verification of the asymptotic formula for $V_0'(L)$
against exact transfer-matrix data for $L = 3$--$14$ confirms the
prediction to $< 0.1\%$ residuals.
\end{abstract}
...
We derive (conditional on Assumption~A of \cref{rem:proof-status}; see \cref{thm:main}):
\[
...
\begin{theorem}[Main result (Conditional on Assumption~A)]
\label{thm:main}
Assume that the $\sigma$-sector vertex function $C(J,L)$ defined
in~\eqref{eq:vertex-bubble} satisfies the finite-size RG scaling
$C(J,L) \sim L^{-2\gamma_\phi}$ with anomalous dimension
$\gamma_\phi$ given by standard Ising CFT data
\textnormal{(Assumption~A; see Remark~\ref{rem:proof-status})}.
Then for the 2D Ising model on the $L \times L$ torus at $J = \Jc$,
...
\begin{remark}[Reconciling the two $\lambda_2$ scalings]
\label{rem:lambda2-reconciliation}
Two distinct scalings of $\lambda_2(k_{\min})$ appear in this paper
and should not be confused.
Equation~\eqref{eq:lambda2-dispersion} gives $\lambda_2(k_{\min})
= C(J)\cdot\gamma(k_{\min})^2$, where the bare dispersion
$\gamma(k_{\min}) \sim 2\pi/L$ yields the geometric factor
$\lambda_2^{(0)} \sim L^{-2}$.
Equation~\eqref{eq:lambda2-full-scaling} gives $\lambda_2(k_{\min})
\propto k_{\min}^{d\nu+\eta} = (2\pi/L)^{d\nu+\eta}$, hence
$\lambda_2 \sim L^{-9/4}$.
These are consistent: the second scaling \emph{incorporates}
the critical enhancement of the vertex function $C(J)$ at $\Jc$,
which itself scales as $C(\Jc) \sim L^{-1/4}$ due to the anomalous
dimension $\eta = 1/4$.
Equation~\eqref{eq:lambda2-dispersion} records the leading geometric
factor; equation~\eqref{eq:lambda2-full-scaling} is the full
finite-size scaling result from RG theory (standard finite-size
scaling~\cite{Privman1990,CardyBook1996}), an input from critical
RG theory rather than a consequence of the bare dispersion alone.
\end{remark}
...
\medskip
\noindent\textit{Physics Argument (Callan--Symanzik extraction).}
The $\sigma$-sector contribution to $C'/C$ is extracted via the
Callan--Symanzik equation for the vertex function. This is a
standard heuristic RG argument in critical phenomena; a fully
rigorous treatment would require deriving equation~\eqref{eq:RG-exponent}
directly from the $(-+)$-sector six-fermion Pfaffian asymptotics
(Gap~A; see Remark~\ref{rem:proof-status}).

At the critical point, $C(J,L)$ satisfies the finite-size
...
dimension of the bond field. The applicability of this equation to
the composite finite-size quantity $C(J,L)$ is the content of
Assumption~A (see Remark~\ref{rem:proof-status}): it assumes
$C(J,L)$ is RG-covariant in the standard sense.
At $J = \Jc$, the $\beta$-function vanishes and
$L\,\partial_L C = -2\gamma_\phi\,C$, giving
$C \sim L^{-2\gamma_\phi}$. The $J$-derivative adds
...
\begin{remark}[Status of the proof and Assumption~A]
\label{rem:proof-status}
Steps~1--3 and the FH input (Step~2) are rigorous.
Step~4 is a heuristic RG argument (labeled as ``Physics Argument''
above), which is standard in critical phenomena but is \emph{not}
a mathematical proof in the strict sense.

\textbf{Assumption~A} (required for \cref{thm:main}):
The $\sigma$-sector vertex function $C(J,L)$ defined
in~\eqref{eq:vertex-bubble} is RG-covariant on the torus, i.e.,
it satisfies the Callan--Symanzik equation~\eqref{eq:callan-symanzik}
with the standard bond-field anomalous dimension $\gamma_\phi$
of the 2D Ising CFT.

This is the single analytical gap in the derivation.
A fully rigorous closure would derive~\eqref{eq:RG-exponent}
directly from the $(-+)$-sector six-fermion Pfaffian asymptotics.
...
We note that $\tfrac{d}{dJ}\log\operatorname{Pf}(G_{6\times 6})\big|_{J=J_c} = -6$
exactly (L-independent), as follows from the Jacobi formula applied to the
antisymmetric propagator matrix at uniform coupling (verified by exact rational
arithmetic using Sage).  The anomalous scaling $L^{2/9}$ therefore does not
reside in the bare Pfaffian vertex but arises from the convolution of this
constant vertex with the critical spectrum of $\hat{\mathrm{F}}^{-1}(k)$ in
the Brillouin-zone sum~\eqref{eq:szego-identity}: specifically, the $\sigma$-channel
eigenvalue $c_2(k_{\min};L)$ diverges as $\chi_\sigma \sim L^{7/4}$ while a
large cancellation in the $c_1$ channel subtracts the naive $L^1$ behaviour,
leaving the compound ratio $L^{2/9}$.  A self-contained replication package
for the numerical verification is available at
\url{https://github.com/Vibecodium/fisher-curvature-replication}.
\end{remark}
...
($V_0'(14) = 14.020$, all $63$ independent BZ modes), confirming the
asymptotic prediction $14.01 \pm 0.08$ to within the
correction-to-scaling uncertainty.
...
The derivation of \cref{thm:main} is complete modulo Assumption~A
(see Remark~\ref{rem:proof-status}): the rigorous derivation of the
$\sigma$-sector six-fermion Pfaffian asymptotics on the torus
(\cref{prop:vertex-scaling}, Step~2).
The physical argument --- $\Z_2$ fusion rules eliminating the energy
channel, Callan--Symanzik RG at the Wilson--Fisher fixed point ---
produces the exponent $\alpha_f = 2/9$ unambiguously.
The asymptotic formula for $V_0'(L)$ is verified to $< 0.1\%$
for $L = 4$--$14$ (Table~\ref{tab:verification}).
The $\sigma$-sector Pfaffian asymptotics (Gap~A) have \emph{not}
been directly computed; the formula is validated as a composite
observable (the fit of $V_0'$), not as a direct Pfaffian calculation. Closing this gap rigorously amounts to establishing the
