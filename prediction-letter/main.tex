...
multi-seed MCMC through $L = 20$ (all effective exponents within $1.7\sigma$).
...
accumulation for 2D at $L = 10$--$20$, and GPU-accelerated MCMC
...
$L = 3$--$9$ and multi-seed MCMC data for $L = 10$--$20$
(Wolff-cluster sampling with FFT cumulant accumulation, 100k--500k
...
The MCMC data at $L = 10$--$20$ extend the test into the regime where
the formula $\dR = (d\nu + 2\eta)/(d\nu + \eta)$ predicts purely
asymptotic scaling. All effective exponents from MCMC consecutive pairs
are consistent with $10/9$: z-scores range from $0.28$ to $1.62$ (all
below $1.7\sigma$). A systematic MCMC bias of $1$--$3\%$ (growing with
...
\caption{\label{fig:scaling}
  ...
  MCMC ($L=10$--$20$, 3 seeds each). Solid line: $\dR = 10/9$ prediction.
...
\begin{enumerate}
  ...
  \item For 2D Ising: exact TM ($L = 6$--$9$) confirms the prediction
    at $1.8\sigma$; multi-seed MCMC ($L = 10$--$20$) confirms all
...
cumulant accumulation (2D Ising $L = 10$--$20$, Intel i9-14900KF +
