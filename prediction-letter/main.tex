...
computations on tori $L = 3$--$8$ (six points, measured $\dR = 1.1110 \pm 0.001$).
For 3D Ising ($\nu = 0.630$, $\eta = 0.0363$), the prediction $\dR = 1.0188$
is consistent with MCMC results on $L^3$ tori $L = 4$--$7$
($d_\mathrm{eff}(6{\to}7) = 1.043 \pm 0.016$, $z = 1.52$). Numerical evidence
for 2D Potts $q = 3$ (predicted $33/29 \approx 1.138$) and $q = 4$
(predicted $22/19 \approx 1.158$) shows $d_\mathrm{eff}$ converging toward predictions.
...
consistency check. This exponent is distinct from the Ruppeiner thermodynamic
...
\subsection{2D Ising: exact transfer matrix}

Table~\ref{tab:ising2d} presents $|\Scal(\Jc)|$ for the 2D Ising model
on $L \times L$ tori computed by exact transfer matrix at the Onsager
critical coupling $\Jc = \frac{1}{2}\ln(1+\sqrt{2})$.
The effective exponent
\begin{equation}
  d_\mathrm{eff}(L, L') = \frac{\ln(|\Scal(L')|/|\Scal(L)|)}{\ln(n'/n)},
  \label{eq:deff}
\end{equation}
converges monotonically from above as $L$ increases.
The last two consecutive pairs give $d_\mathrm{eff}(6{\to}7) = 1.1113$ and
$d_\mathrm{eff}(7{\to}8) = 1.1110$, a change of only $0.0003$.
We quote the asymptotic value $\dR = 1.1110 \pm 0.001$, in agreement
with the theoretical prediction $10/9 = 1.1\overline{1}$ to within $0.01\%$.

\begin{table}[b]
\caption{\label{tab:ising2d}
  Fisher scalar curvature $|\Scal(\Jc)|$ for 2D Ising on $L\times L$ tori (PBC),
  computed by exact transfer matrix. $m = 2L^2$ is the number of bonds.
  Effective exponents $d_\mathrm{eff}$ converge to the predicted $10/9 = 1.1111$.}
\begin{ruledtabular}
\begin{tabular}{rrrrl}
  $L$ & $n=L^2$ & $m=2L^2$ & $|\Scal|$ (exact) & $d_\mathrm{eff}$ \\
  \colrule
  3 & 9  & 18  & 105.6418  & --- \\
  4 & 16 & 32  & 265.3121  & 1.6005 \\
  5 & 25 & 50  & 446.2378  & 1.1650 \\
  6 & 36 & 72  & 671.2472  & 1.1197 \\
  7 & 49 & 98  & 945.5316  & 1.1113 \\
  8 & 64 & 128 & 1272.1419 & 1.1110 \\
\end{tabular}
\end{ruledtabular}
\end{table}
